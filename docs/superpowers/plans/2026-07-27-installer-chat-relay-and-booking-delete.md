...
- **Known accepted edges (documented in code comments):** dispatch requires the @ prefix sent alone; any @-prefixed text from a dispatcher is treated as command intent and is consumed (help SMS on malformed input) — bare words always relay as chat text; customer-facing escalation copy now names the dispatcher (the business's primary contact) rather than the market installer.
