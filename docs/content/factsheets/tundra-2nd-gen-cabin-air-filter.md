# FACT SHEET — Tundra 2nd Gen (2007–2021) Cabin Air Filter
...
## 1. FSM Findings (verbatim + page)
⚠️ **Applicability flag:** the 2007 FSM lists the cabin air filter as **"if equipped"** (p.2242) — early 2nd-gen Tundras did not all have a factory cabin filter; many owners retrofit one into the empty HVAC slot behind the glovebox. The video should open by helping the viewer check if theirs is equipped.
...
## 3. Prop Shot List
Rocky holds a **cabin filter element**; Octane by the glovebox. Angle the episode as "check + upgrade."

## 4. Sign-Off Gate
- [ ] Confirm which years/trims came equipped vs not (owner-verify) — key to the video's premise
- [ ] Source written procedure (FSM is diagram-only) or script from glovebox method
- [ ] Confirm TY-orderable filter (or omit CTA)
- [ ] Aaron approves
