...
2. **Rebook report** — new pure `lib/rebook-render.js` `renderRebookReport(records,{title})` (all + grouped by City + grouped by installer). Sent to info@ two ways: **T+1 per event** (hook in `event-reminders.js` after the sweep) + **weekly Mondays** (new scheduled fn `rebook-report.js`, cron `0 13 * * 1`, all outstanding Priority rows where `!Notified`). **RENAMED 2026-07-04 (master @ b50e015):** the per-event report title is now **"Post-Event Summary — &lt;City&gt; (&lt;event date&gt;)"** (names the swept event + date, not just today's date); the weekly one stays "Weekly rebook backlog".
...
**Model year on the emails — 2026-07-04 (master @ b50e015).** As part of [[booking-model-year-capture]], the exact model year now appears as ` (YYYY)` appended after the vehicle (omitted when blank) in the **installer roster** (`roster-render.js` Vehicle cell), the **Post-Event Summary + Weekly rebook backlog** (`rebook-render.js` `line()`), and the **certificate** (both the immediate close-out cert in `installer-closeout.js` AND the daily backstop — the immediate path was missing `modelYear` before this). The swept Priority record now also carries `Model Year` (tolerant), so the weekly backlog has it too. Data source: book.js writes `Model Year` to Bookings/Priority; renderers read `r["Model Year"]`.

**Reason key legend — 2026-07-04 (master @ b68876d + 3b5a71f).** A "Reason key" legend now prints on the rebook emails (Post-Event Summary + Weekly backlog, always) and the **installer roster** (only when a waitlist exists — the only place a Reason shows there). Single source of truth = **`netlify/functions/lib/reasons.js`** (`REASON_KEY` + `reasonKeyText()`/`reasonKeyHtml()`), imported by both `rebook-render.js` and `roster-render.js` so the three surfaces + SOP 5 never drift. **The 3 stored `Reason` values are `Rebook — not completed` / `Event full` / `No event scheduled`** (from `event-plan.js` SWEEP_REASON + `book.js` priority path) — SOP 5's table was WRONG (listed internal keys `no-event`/`full`) and was corrected to these exact values. If you change a Reason string, update `reasons.js` + `book.js`/`event-plan.js` + SOP 5 together.

**Close-out → Completed (the ONLY manual step; everything downstream is automatic).** Installer opens `/installer.html`, enters passcode, taps **Mark complete** (with OTT Calibration + VIN) → `installer-closeout.js` sets `Status:Completed` + emails the cert immediately. SOP: `docs/operations/sop-event-closeout.md` (SOP 4). Backstops: `certificate-dispatch.js` daily ~9AM re-sends any Completed cert not yet sent; the −1d sweep rolls any non-Completed booking into Priority → the Post-Event Summary. There is no auto-complete (only the installer knows the vehicle was actually tuned).
