...
- **"Get OTT Now!" shareable funnel — SHIPPED LIVE 2026-07-01 (master @ d5cf5a2).** Spec/plan: `docs/superpowers/{specs,plans}/2026-07-01-get-ott-now-share*`. Clean **`/get-ott-now`** link → Netlify 302 redirect (in `netlify.toml`) into the existing funnel with `utm_source=share&utm_medium=social&utm_campaign=get-ott-now`, so shared-link leads are attributed `get-ott-now` (funnel already reads UTM → book.js writes to Airtable UTM cols; routes to booking or priority list unchanged). Homepage hero: "Get OTT Now! →" CTA + a **Share** button; funnel page: a "Share 'Get OTT Now'" link. Share logic in shared **`site/share.js`** (`[data-share-ott]` triggers) — `navigator.share` native sheet on mobile (covers SMS/email/FB/IG/Reddit/TikTok/YouTube), desktop fallback popover (Copy Link + Text/Email/Facebook/Reddit + "paste into IG/TikTok/YouTube" note; those 3 have no web share-to-DM intent). Node-testable pure `shareLinks(url,text)`. 209 tests green; verified live (302 correct, homepage CTA present, share.js 200). No per-platform attribution (native sheet shares one URL).
