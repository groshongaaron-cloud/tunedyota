...
const { ecuCandidates, is3rdGenTacoma, is3rdGenTacoma35, defaultGear, gearForTransmission } = require("../netlify/functions/lib/ecu-ids.js");
...
test("the report break-out section is 3.5L-only (2.7L 3rd gen gets 3.90 gear but is NOT in the section)", () => {
  assert.equal(is3rdGenTacoma35({ vehicleType: "Tacoma", engine: "3.5", year: 2019 }), true);
  assert.equal(is3rdGenTacoma35({ vehicleType: "Tacoma", engine: "2.7", year: 2019 }), false);  // 2.7L excluded from section
  assert.equal(defaultGear({ vehicleType: "Tacoma", engine: "2.7", year: 2019 }), "3.90");       // ...but still gets 3.90 gear
});
