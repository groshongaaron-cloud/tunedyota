...
test("delivers Slack summary + single owner Monthly Executive Summary with contacts.csv", async () => {
  const d = deps();
  await runReport(d);
  assert.equal(d._notifies.length, 1);
  assert.match(d._notifies[0].text, /Submissions Digest/);
  // Exactly one email — the owner-facing executive summary. No per-installer fan-out.
