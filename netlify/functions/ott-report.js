...
const { priorMonth, buildSubmissionRows, renderOttXlsx, renderOwnerDraftHtml, totalCommission, unresolved, leadConversion } = require("./lib/ott-report.js");
...
async function runOttReport(deps) {
  ...
  const subRows = buildSubmissionRows(flatBookings, month, { retailer: env.OTT_RETAILER, sendDate: "" });
  const conversion = leadConversion(flattenRecords(pRecs), flatBookings, month);

  if (!subRows.length) {
    const convLine = conversion.received > 0 ? ` · OTT: ${conversion.received}→${conversion.booked}→${conversion.completed}` : "";
    try { await notify({ fetchImpl, webhookUrl: env.SLACK_WEBHOOK_URL, text: `Tuned Yota — OTT commission draft (${month.label}): 0 completed calibrations — nothing to approve.${convLine}`, log }); }
  ...
  try {
    await send({ fetchImpl, apiKey: env.RESEND_API_KEY, from: FROM, to: OWNER, replyTo: OWNER,
      ...
      html: renderOwnerDraftHtml(subRows, month, approveUrl(env, month.key), conversion),
  ...
  slack += ` · OTT: ${conversion.received}→${conversion.booked}→${conversion.completed}`;
