...
async function runAmsoilFollowup(deps) {
  ...
  for (const row of rows) {
    ...
    try {
      const { subject, html, text } = buildAmsoilEmail({ name: f.Name, vehicle: f.Vehicle, modelYear: f["Model Year"], fluids, bookingId: row.id, accountUrl: accountLink(f.Email, Date.now(), env), reviewUrl: env.GOOGLE_REVIEW_URL || "" });
